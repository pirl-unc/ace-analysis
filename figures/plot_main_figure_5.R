...
PlotPrecision <- function(df.plot, title) {
  df.plot$text_color <- ifelse(df.plot$precision < 0.5, "black", "white")
  plot <- ggplot(df.plot, aes(x = num_peptides_per_pool, y = num_coverage, fill = precision)) +
    ...
    scale_fill_gradient(low = PRECISION.LOW.HEX, high = PRECISION.HIGH.HEX, limits = c(0,1)) +
...
PlotNumTotalPools <- function(df.plot, title, min, max) {
  ...
  plot <- ggplot(df.plot, aes(x = num_peptides_per_pool, y = num_coverage, fill = num_total_pools)) +
    ...
    xlab("Number of Peptides per Pool") + ylab("Coverage") + ggtitle(title) +
    scale_fill_gradient(low = NUM.TOTAL.POOLS.LOW.HEX, high = NUM.TOTAL.POOLS.HIGH.HEX, limits = c(min,max)) +
    scale_x_continuous(breaks = seq(5,40,1)) +
    scale_y_continuous(breaks = seq(2,10,1)) +
    scale_color_manual(values = c('black', 'white'))+
    theme_pubr() + COMMON.THEME
...
df.plot <- df %>%
  ...
  filter(num_peptides_per_pool <= 40) %>%
...
ggsave(filename = paste0(OUTPUT.DIR, "/main_figure_5.pdf"), width = 24, height = 24, dpi = 300)

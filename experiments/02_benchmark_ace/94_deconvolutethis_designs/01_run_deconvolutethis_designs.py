...
OUTPUT_DIR = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/01_benchmark_ace/04_deconvolutethis_designs'
...
if __name__ == "__main__":
    ...
    for index, row in df_experiments.iterrows():
        ...
        df_results.to_csv(
            '%s/deconvolutethis_designs_experiment_results_%i.tsv' % (output_dir, experiment_id),
            sep='\t',
            index=False
        )
        df_assignments.to_csv(
            '%s/deconvolutethis_designs_experiment_assignments_%i.tsv' % (output_dir, experiment_id),
            sep='\t',
            index=False
        )

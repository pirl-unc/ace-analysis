...
NUM_PROCESSES = 64
NUM_ITERATIONS = 100
REFERENCE_PEPTIDES_CSV_FILE = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/raw/tiling_window_covid_spike_9mers.csv'
...
OUTPUT_DIR = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/01_benchmark_ace/06_real_world_datasets/tiling_window_covid_spike_9mers'
RANDOM_EFFECTS = True
...
if __name__ == "__main__":
    ...
    print('Started running an experiment on COVID spike protein 9mers (tiling window) dataset')
    num_peptides = 1265
    ...
    df_results.to_csv(
        '%s/tiling_window_covid_spike_9mers_experiment_results_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )
    df_assignments.to_csv(
        '%s/tiling_window_covid_spike_9mers_experiment_assignments_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )
    print('Finished running the above experiment.')
    print('Finished running an experiment on COVID spike protein 9mers (tiling window) dataset')

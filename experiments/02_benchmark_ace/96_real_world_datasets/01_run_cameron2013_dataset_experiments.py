...
OUTPUT_DIR = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/01_benchmark_ace/06_real_world_datasets/cameron_et_al_sci_trans_med_2013'
RANDOM_EFFECTS = True
...
if __name__ == "__main__":
    ...
    df_results.to_csv(
        '%s/cameron_et_al_sci_trans_med_2013_experiment_results_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )
    df_assignments.to_csv(
        '%s/cameron_et_al_sci_trans_med_2013_experiment_assignments_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )

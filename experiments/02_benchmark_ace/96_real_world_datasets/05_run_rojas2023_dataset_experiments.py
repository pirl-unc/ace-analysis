...
NUM_PROCESSES = 64
NUM_ITERATIONS = 1000
REFERENCE_PEPTIDES_CSV_FILE = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/raw/rojas_et_al_nature_2023.csv'
...
OUTPUT_DIR = '/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/01_benchmark_ace/06_real_world_datasets/rojas_et_al_nature_2023'
RANDOM_EFFECTS = True
...
if __name__ == "__main__":
    ...
    print('Started running an experiment on Rojas et al., Nature 2023, dataset')
    num_peptides = 232
    num_peptides_per_pool = 8
    num_coverage = 3
    num_true_positive_peptides = 30
    ...
    df_results.to_csv(
        '%s/rojas_et_al_nature_2023_experiment_results_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )
    df_assignments.to_csv(
        '%s/rojas_et_al_nature_2023_experiment_assignments_%i.tsv' % (output_dir, experiment_id),
        sep='\t',
        index=False
    )
    print('Finished running the above experiment.')
    print('Finished running an experiment on Rojas et al., Nature 2023, dataset')
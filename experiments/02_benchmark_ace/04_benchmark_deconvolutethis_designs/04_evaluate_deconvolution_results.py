...
DECONVOLUTION_RESULTS_DIR = "/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/02_benchmark_ace/04_benchmark_deconvolutethis_designs/03_deconvolution_results/300immunogenic_30nonimmunogenic_0dispersion_fnr0.00"
OUTPUT_DIR = "/datastore/lbcfs/collaborations/pirl/members/jinseok/projects/project_ace/data/processed/02_benchmark_ace/04_benchmark_deconvolutethis_designs/04_evaluation_results/300immunogenic_30nonimmunogenic_0dispersion_fnr0.00"
...
if __name__ == '__main__':
    print("Started")
    tasks = []
    for directory in glob.glob(DECONVOLUTION_RESULTS_DIR + '/*'):
        subdir_name = directory.split('/')[-1]
        for deconvolution_results_tsv_file in glob.glob(directory + '/ace-s/ace_cem/*pool_spot_counts*.tsv'):
            deconvolution_results_tsv_file_basename = os.path.basename(deconvolution_results_tsv_file)
            peptides_tsv_file = SAMPLED_PEPTIDES_DIR + '/' + '_'.join(deconvolution_results_tsv_file_basename.split('_')[0:3]) + '.tsv'
            configuration_tsv_file = glob.glob(CONFIGURATIONS_DIR + '/ace-s/' + subdir_name + '/' + '_'.join(deconvolution_results_tsv_file_basename.split('_')[0:5]) + '*.tsv')[0]
    ...
    df_evaluation_metrics_all.to_csv(OUTPUT_DIR + '/configuration_ace-s_deconvolution_ace-cem_evaluation_metrics.tsv', sep='\t', index=False)
    df_evaluation_results_all.to_csv(OUTPUT_DIR + '/configuration_ace-s_deconvolution_ace-cem_evaluation_results.tsv', sep='\t', index=False)

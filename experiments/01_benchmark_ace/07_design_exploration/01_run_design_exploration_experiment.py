...
if __name__ == "__main__":
    ...
    for index, row in df_experiments.iterrows():
        ...
        num_true_positive_peptides = row['num_true_positives']
        ...
        df_results.to_csv(
            '%s/design_exploration_experiment_results_%i.tsv' % (output_dir, experiment_id),
            sep='\t',
            index=False
        )
        df_assignments.to_csv(
            '%s/design_exploration_experiment_assignments_%i.tsv' % (output_dir, experiment_id),
            sep='\t',
            index=False
        )

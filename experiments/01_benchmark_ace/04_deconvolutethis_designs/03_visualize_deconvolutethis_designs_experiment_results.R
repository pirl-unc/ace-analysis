...
MERGED.TSV.FILE <- "/Users/leework/Documents/Research/projects/project_ace/data/processed/01_benchmark_ace/deconvolutethis_designs/deconvolutethis_experiment_results_merged.tsv"
OUTPUT.DIR <- "/Users/leework/Documents/Research/projects/project_ace/data/processed/01_benchmark_ace/deconvolutethis_designs"
DODGE.WIDTH <- 0.5
ERRORBAR.WIDTH <- 0.263
BOXPLOT.WIDTH <- 0.384
SOLVER.COLORS <- c("Golfy (+Clustering / -Extra Pools)" = "#845EC2",
                   "Golfy (-Clustering / -Extra Pools)" = "#D65DB1",
                   "DeconvoluteThis" = "#FF9671")
...
df.plot.temp.2 <- df.plot %>%
  dplyr::group_by(group, solver, num_positive_peptide_sequences) %>%
  dplyr::summarise(
    sd = sd(predicted_total_pools, na.rm = T),
    predicted_total_pools = mean(predicted_total_pools)
  )

p <- ggplot(df.plot, aes(x = ))
...
for (group in unique(df.plot$group)) {
  ...
  plot.num.total.pools <- ggplot(df.plot.temp.2, aes(perc_positive_peptide_sequences, predicted_total_pools, colour = solver)) +
    geom_line(width = 1.618) +
    geom_errorbar(aes(ymin = predicted_total_pools - sd, ymax = predicted_total_pools + sd), width = 0.146) +

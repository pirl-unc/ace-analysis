...
df.precision.recall.aucroc.numpools <- df.precision.recall.aucroc.numpools %>%
  mutate(deconvolution_method = ifelse(deconvolution_method == "strandberg_bayesian", "strandberg-bayesian", deconvolution_method))
...
df.evaluation.results <- df.evaluation.results %>%
  dplyr::filter(group %in% c("ace-s_ace-cem",
                             ...
                             "strandberg_strandberg-empirical"))
df.evaluation.results <- df.evaluation.results %>%
  mutate(group = ifelse(group == "strandberg_strandberg_bayesian", "strandberg_strandberg-bayesian", group))
...
df.aucprc.all <- df.aucprc %>%
  dplyr::group_by(modnames, num_peptides) %>%
...
df.aucprc.all <- rbind(df.aucprc.all,
                       data.frame(
                         modnames = c("deconvolutethis", "deconvolutethis"),
                         num_peptides = c(120, 800),
                         aucprc = c(NA, NA),
                         configuration_method = c("deconvolutethis", "deconvolutethis"),
                         deconvolution_method = c("deconvolutethis", "deconvolutethis")
                       ))

df.main.table.1 <- merge(df.precision.recall.aucroc.numpools, df.aucprc.all, by = c("configuration_method", "deconvolution_method", "num_peptides"))
write.table(x = df.main.table.1, file = paste0(OUTPUT.DIR, "/main_table_1.tsv"), sep = "\t", row.names = F)
